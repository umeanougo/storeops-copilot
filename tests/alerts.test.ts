...
describe("multi-merchant deterministic alert engine",()=>{
  ...
  it("does not treat unavailable inventory as zero",()=>{const{snapshot}=setup();const variant=snapshot.products[0].variants[0];variant.available=null;const alerts=detectAlerts(snapshot,DEFAULT_THRESHOLDS);expect(alerts.some(item=>item.issueType==="inventory_constraint"&&item.supportingData.some(fact=>fact.recordId===variant.id))).toBe(false);expect(alerts.some(item=>item.issueType==="low_inventory"&&item.recordId===variant.id)).toBe(false);});
  it("does not flag low inventory for an archived product",()=>{const{snapshot}=setup();const product=snapshot.products[0];product.status="ARCHIVED";expect(detectAlerts(snapshot,DEFAULT_THRESHOLDS).some(item=>item.issueType==="low_inventory"&&item.recordId===product.variants[0].id)).toBe(false);});
